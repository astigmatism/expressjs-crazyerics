/*
    Virtual Boy Vintage LCD v2

    A single-pass, resource-free LCD/dot-matrix shader tuned for Nintendo
    Virtual Boy's red-on-black image. It avoids RGB subpixel simulation,
    external textures, and multi-pass history buffers.

    This version deliberately samples the RetroArch source texture at TEX0.xy
    instead of snapping the sample coordinate to a TextureSize-derived texel
    center. Browser fullscreen can change the relationship between RetroArch's
    source texture, active viewport, canvas backing store, and CSS size. Snapping
    the source lookup from TextureSize can then push lookups onto a repeated edge
    texel, which appears as long red horizontal smears in fullscreen. The LCD
    mask is still based on source-pixel coordinates, but the picture sample stays
    on RetroArch's own texture coordinate.
*/
...
void main()
{
    ...
    COMPAT_PRECISION vec2 maskSize = max(min(textureSize, inputSize), vec2(1.0));

    COMPAT_PRECISION float inBounds = step(0.0, texCoord.x) * step(0.0, texCoord.y) *
        step(texCoord.x, 1.0) * step(texCoord.y, 1.0);

    COMPAT_PRECISION vec4 source = COMPAT_TEXTURE(Texture, texCoord) * inBounds;
    COMPAT_PRECISION vec2 pixelCoord = texCoord * maskSize;
