...
var cesEmulator = (function(_Compression, _PubSub, _config, _Sync, _GamePad, _Preferences, _gameKey, _Logging) {
    ...
    var module = (function() {
        ...
        this.setStatus = function(text) {
            ...
            console.log(text);
            if (text === 'Running...') {
                _PubSub.Publish('emulatorseemsready', null, true); //true, sustained
            }
            return;
        ...
        this.cesWriteFile = function(parent, filename, contents, callback) {
            ...
            var result = this.FS.createDataFile(parent, filename, contents, true, true);
        ...
        this.BuildLocalFileSystem = function(gameKey, compressedGameData, compressedSupprtData, compressedShaderData) {
            ...
            this.FS.createFolder('/', 'system', true, true);
            if (compressedSupprtData) {
                for (var supportFile in compressedSupprtData) {
                    ...
                    try {
                        this.FS.createDataFile('/system', filename, content, true, true);
                    } catch (e) {
            ...
            this.FS.createFolder('/', 'games', true, true);
            ...
            if (compressedGameData.hasOwnProperty('f')) {
                for (var gameFile in compressedGameData.f) {
                    ...
                    this.FS.createDataFile('/games', filename, gamedata, true, true);
                }
            }
            ...
            //shaders
            this.FS.createFolder('/', 'shaders', true, true);
            ...
            if (compressedShaderData && compressedShaderData.hasOwnProperty('f')) {
                for (var shaderFile in compressedShaderData.f) {
                    ...
                    //write to emaultor
                    try {
                        this.FS.createDataFile('/shaders', filename, content, true, true);
                    } catch (e) {
                        //an error on file write.
                    }
            ...
            try { this.FS.createFolder('/', 'home', true, true); } catch (e) {}
            try { this.FS.createFolder('/home', 'web_user', true, true); } catch (e) {}
            try { this.FS.createFolder('/home/web_user/', 'retroarch', true, true); } catch (e) {}
            try { this.FS.createFolder('/home/web_user/retroarch', 'userdata', true, true); } catch (e) {}

            //the config isnt any different from 1.6.9 so i just take it
            if (_config.retroarch['1.6.9-stable']) {
                ...
                this.FS.createDataFile('/home/web_user/retroarch/userdata', 'retroarch.cfg', configString, true, true);
            }

            //screenshots
            this.FS.createFolder('/', 'screenshots', true, true);

            //state save location
            this.FS.createFolder('/', 'states', true, true);

            //save file location
            this.FS.createFolder('/', 'saves', true, true);
        };
